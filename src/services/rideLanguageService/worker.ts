...
const worker = (() => {
    ...
    function compileRideFile(content: string, needCompaction: boolean, removeUnused: boolean, libraries?: Record<string, string>) {
        ...
        try {
            ...
            const compilationResult: IFlattenedCompilationResult = flattenCompilationResult(
                RideJS.compile(content, 3, needCompaction, removeUnused, libraries)
            );
    ...
    self.addEventListener('message', (e) => {
        ...
        try {
            ...
            result = { compilation: { error: String(e) } };
        }

        self.postMessage({ result, msgId });
    });
});
