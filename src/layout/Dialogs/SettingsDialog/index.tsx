...
import { AccountsStore, IAccount, INode, SettingsStore, TFile } from '@stores';
...
import ImportTree from '@src/layout/Dialogs/SettingsDialog/ImportTree';
...
export const SectionHead: React.FunctionComponent = (props) => <div className={styles.section_head}>{props.children}</div>;
...
export default class SettingsDialog extends React.Component<IProps, IState> {
    ...
    handleSetLoadedState = (data: IImportedData) => {
        console.log(data);
        data.accounts && data.customNodes && data.files && this.setState({data});
    };
        ...
        return <>
            ...
            >
                <Scrollbar className={styles.content}>
                    {this.state.data
                        ? <ImportTree data={this.state.data}/>
                        : <>
                            <Row>
                                <Section>
